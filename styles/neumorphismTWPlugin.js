...
module.exports = plugin(
    function ({ addUtilities, e, theme, variants }) {
        ...
        _.forEach(
            flattenColorPalette(
                theme("neumorphismColor", theme("backgroundColor"))
            ),
            (color, colorKey) => {
                ...
                _.forEach(theme("neumorphismSize"), (size, sizeKey) => {
                    nmFlatPairs.push([
                            ...
                            : `.${e(`nm-flat-${colorKey}-${sizeKey}`)}`,
                        {
                            ...
                            boxShadow: `${size} ${size} calc(${size} * 2) ${shades.shadowColor}, calc(${size} * -1) calc(${size} * -1) calc(${size} * 2.5) 1px ${shades.highlightColor}, 
                            inset ${size} ${size} calc(${size} * 2.5) ${shades.highlightColor}, inset calc(${size} * -1) calc(${size} * -1) calc(${size} * 0.6) ${shades.shadowColor}`,
                        },
                    ]);
                });
            }
